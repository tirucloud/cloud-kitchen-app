...
// the HTTP API and the order.placed consumer goroutine, then blocks until
...
import (
	...
	"github.com/cloudkitchen/payment-service/internal/config"
	"github.com/cloudkitchen/payment-service/internal/handler"
	...
	"github.com/cloudkitchen/payment-service/migrations"
...
func main() {
	...
	paymentSvc := service.NewPaymentService(paymentRepo, b, logger)

	if err := b.Consume(
		"payment-service.events",
		[]string{"order.placed"},
		paymentSvc.HandleEvent,
	); err != nil {
	...
	handler.New(paymentSvc, cfg.JWTSecret).Register(router)
