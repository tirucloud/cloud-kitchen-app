...
```
https://thirucloud.shop                — the app UI
https://thirucloud.shop/argocd         — ArgoCD UI
https://thirucloud.shop/grafana        — Grafana
https://thirucloud.shop/prometheus     — Prometheus
https://thirucloud.shop/alertmanager   — Alertmanager
```
...
| Check | How |
|---|---|
| Phase 5 done (your domain resolves to the LB IP) | `dig +short thirucloud.shop` → returns your LB IP (e.g. `136.112.45.103`) |
...
# Try it yourself:
curl -v http://thirucloud.shop/.well-known/acme-challenge/test
...
spec:
  ...
  routes:
    ...
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathRegexp(`^/api/restaurants/[^/]+/(menu|categories|items)`)
      kind: Rule
      priority: 200
      services:
        - {name: menu-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/menu`)
      kind: Rule
      priority: 200
      services:
        - {name: menu-service, port: 8080}
    # --- one rule per service ---
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/auth`)
      kind: Rule
      priority: 100
      services:
        - {name: auth-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/users`)
      kind: Rule
      priority: 100
      services:
        - {name: user-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/restaurants`)
      kind: Rule
      priority: 100
      services:
        - {name: restaurant-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && (PathPrefix(`/api/cart`) || PathPrefix(`/api/orders`))
      kind: Rule
      priority: 100
      services:
        - {name: order-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/payments`)
      kind: Rule
      priority: 100
      services:
        - {name: payment-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/deliveries`)
      kind: Rule
      priority: 100
      services:
        - {name: delivery-service, port: 8080}
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/api/notifications`)
      kind: Rule
      priority: 100
      services:
        - {name: notification-service, port: 8080}
    # --- frontend catch-all (lowest priority) ---
    - match: (Host(`thirucloud.shop`) || Host(`136.112.45.103`)) && PathPrefix(`/`)
      kind: Rule
      priority: 1
      services:
...
```bash
curl -sI "https://thirucloud.shop/" | head -1
# Expect: HTTP/2 200

curl -s "https://thirucloud.shop/api/restaurants" | head -c 200 ; echo
# Expect: JSON array of restaurants

# All 10 microservice routes still work — try a couple more:
curl -s -o /dev/null -w "HTTP %{http_code}\n" "https://thirucloud.shop/api/auth/health"
curl -s -o /dev/null -w "HTTP %{http_code}\n" "https://thirucloud.shop/api/menu"
```
...
Right now those four UIs are reachable at `http://thirucloud.shop/argocd/`,
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`thirucloud.shop`) && PathPrefix(`/grafana`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`thirucloud.shop`) && PathPrefix(`/prometheus`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`thirucloud.shop`) && PathPrefix(`/alertmanager`)
      kind: Rule
      services:
...
spec:
  entryPoints:
    - websecure
  routes:
    - match: Host(`thirucloud.shop`) && PathPrefix(`/argocd`)
      kind: Rule
      services:
        - name: argocd-server
          port: 80
...
- https://thirucloud.shop/                  → React UI
- https://thirucloud.shop/argocd/           → ArgoCD UI (admin / your bootstrap password)
- https://thirucloud.shop/grafana/          → Grafana (admin / prom-operator)
- https://thirucloud.shop/prometheus/       → Prometheus
- https://thirucloud.shop/alertmanager/     → Alertmanager
...
If you `curl http://thirucloud.shop/` right now you'll see:
...
for path in / argocd/ grafana/login prometheus/-/ready alertmanager/-/ready; do
  ...
  curl -sI "http://thirucloud.shop/${path#/}" \
    | awk '/^HTTP|^[Ll]ocation:/ {printf "%s ", $0}' ; echo
done
# Expect for each:  HTTP/1.1 308 Permanent Redirect  Location: https://thirucloud.shop/...

# And follow the redirect — should land on 200 over HTTPS
for path in / argocd/ grafana/login prometheus/-/ready alertmanager/-/ready; do
  printf "%-30s -> %s\n" "http://...${path}" \
    "$(curl -sIL -o /dev/null -w '%{http_code} %{url_effective}' "http://thirucloud.shop/${path#/}")"
done
# Expect:  200 https://thirucloud.shop/...  for every path
```
...
|---|---|---|
| `Certificate` stuck `READY=False` for >5 min | Let's Encrypt HTTP-01 can't reach `http://thirucloud.shop/.well-known/acme-challenge/…` | Confirm port 80 is open: `curl http://thirucloud.shop/.well-known/acme-challenge/test` should return 404 (not "connection refused"). `kubectl get challenges -A` shows the exact URL Let's Encrypt is probing — curl it from your laptop. |
| Browser shows "NET::ERR_CERT_AUTHORITY_INVALID" or warns | Either the certificate isn't ready yet OR you're hitting Traefik via an IP without the matching `Host:` header (Traefik then serves a self-signed default cert) | Confirm `kubectl -n cloudkitchen get certificate cloudkitchen-tls` shows `READY=True`, and curl with the hostname (`curl https://thirucloud.shop/`), not the IP. |
...
```
https://thirucloud.shop                📱 the app
https://thirucloud.shop/argocd         🚀 GitOps controller
https://thirucloud.shop/grafana        📊 dashboards
https://thirucloud.shop/prometheus     📈 metrics
https://thirucloud.shop/alertmanager   🚨 alerts
```
